...
export class StationsService {
   ...
   */
  async findOrCreate(eventId: string, rawName: string): Promise<Station> {
    ...
    return this.prisma.station.create({ data: { eventId, name } });
  }

  /** Vérifie qu'une gare existe et appartient bien à l'évènement. */
  async ensureInEvent(eventId: string, stationId: string): Promise<Station> {
